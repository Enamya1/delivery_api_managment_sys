...
def add_worker():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            '''INSERT INTO delivery_workers (name, status, location) 
               VALUES (%s, %s, %s)''', (name, status, location)
        )
        conn.commit()
        cursor.close()
        conn.close()
...
# Route to update a worker's status and location
    ...
    try:
        ...
        cursor.execute(
            '''UPDATE delivery_workers 
               ...
               WHERE id = %s''', (status, location, worker_id)
        )
        conn.commit()
        cursor.close()
        conn.close()
